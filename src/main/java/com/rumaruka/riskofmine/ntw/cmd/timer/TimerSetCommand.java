...
public class TimerSetCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        LiteralCommandNode<CommandSourceStack> literalCommandNode = dispatcher.register(Commands.literal("timer").requires((p_198442_0_) -> {
            ...
            return setTimer(p_198445_0_.getSource(), EntityArgument.getPlayers(p_198445_0_, "targets"), IntegerArgumentType.getInteger(p_198445_0_, "amount"));
        })))));
    ...
    private static int setTimer(CommandSourceStack source, Collection<? extends ServerPlayer> playerEntities, int amount) {
        for (ServerPlayer player : playerEntities) {
            Timer lunar = Timer.get(player);
            lunar.setTimer(amount);


        }
        if (playerEntities.size() == 1) {
            source.sendSuccess(() -> Component.translatable("commands.set." + "timer" + ".success.single", amount, playerEntities.iterator().next().getDisplayName()), true);
