...
public class ROMLargeChestFeature extends Feature<NoneFeatureConfiguration> {


    public ROMLargeChestFeature(Codec<NoneFeatureConfiguration> p_65786_) {
        super(p_65786_);
    }
        ...
        for (Integer integer : intarraylist) {
            for (Integer integer1 : intarraylist1) {
                ...
                if (worldgenlevel.isEmptyBlock(blockpos) || worldgenlevel.getBlockState(blockpos).getCollisionShape(worldgenlevel, blockpos).isEmpty()) {
                    worldgenlevel.setBlock(blockpos, ROMBlocks.LARGE_CHEST.defaultBlockState(), 2);
                    RandomizableContainer.setBlockEntityLootTable(worldgenlevel, randomsource, blockpos, ROMLootTables.LARGE_CHEST);
                    return true;
                }
            }
        }
